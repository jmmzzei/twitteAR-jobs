...
export const submitHandler = (hashtagsInput, keywordsInput, unwantedInput) => {
    ...
    ).map(el => percentEncode(el.innerText.slice(1, -2)))

    let keywords = Array.from(
        document.getElementsByClassName('keyw-span'),
    ).map(el => percentEncode(el.innerText.slice(0, -2)))

    let unwanted = Array.from(
        document.getElementsByClassName('unwanted-span'),
    ).map(el => percentEncode(el.innerText.slice(1, -2)))
